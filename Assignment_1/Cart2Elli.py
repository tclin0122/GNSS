...
ni=np.array([-np.sin(np.deg2rad(lat))*np.cos(np.deg2rad(lon)),-np.sin(np.deg2rad(lat))*np.sin(np.deg2rad(lon)),np.cos(np.deg2rad(lat))])
ei=np.array([-np.sin(np.deg2rad(lon)),np.cos(np.deg2rad(lon)),0])
ui=np.array([np.cos(np.deg2rad(lat))*np.cos(np.deg2rad(lon)),np.cos(np.deg2rad(lat))*np.sin(np.deg2rad(lon)),np.sin(np.deg2rad(lat))])
R=np.array([ni,ei,ui])
print(R)
cnt=0
for i in range (0, 29):
